...
mod test {
    ...
    use snark_verifier_sdk::{
        ...
        halo2::BfvPkEncryptionCircuit::BfvPkEncryptionCircuit,
    };
    use snark_verifier_sdk::CircuitExt;
    ...
    use std::path::Path;
    //use snark_verifier_sdk::evm::{evm_verify, gen_evm_proof, gen_evm_verifier};
    ...
    pub fn test_pk_enc_full_prover() {
        ...
        // 6. Verify the proof
       // check_proof_with_instances(&kzg_params, pk.get_vk(), &proof, &[&instances[0]], true);
        let instances:Vec<Vec<Fr>>  = rlc_circuit.instances();
    ...
    pub fn bench_pk_enc_full_prover() {
        ...
        for config in &configs {
            ...
            // 5. Verify the proof
            let timer = std::time::Instant::now();
            check_proof_with_instances(&config.kzg_params, pk.get_vk(), &proof, &[&instances[0]],
                                       true,);
